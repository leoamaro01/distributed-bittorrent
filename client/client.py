...
class TorrentDownloadStatus:
    ...
    def submit_download(self, file_index: int, part_index: int):
        self.downloading_parts_progress.append(PartDownloadStatus(file_index, part_index))
    
    def __find_part(self, file_index: int, part_index: int, search_in_downloaded: bool = False):
    ...
    def __str__(self) -> str:
        ...
        for d in self.downloading_parts_progress:
            ...
            file_statuses[d.file_index].downloading_parts.append((d.part_index, d.progress, d.source))
        ...
        def get_progress_bar(progress: float):
            ...
            return "[" + "●" * full_segments + "◯" * (10 - full_segments) + f"] ({percentage}%)"
        ...
        for f in file_statuses:
            ...
            downloaded_mb = file_statuses[f].downloaded_parts * self.torrent_info.piece_size / float(1000000)
            ...
            for part, part_progress, source in file_statuses[f].downloading_parts:
                result += f"\tPart {part} {get_progress_bar(part_progress)}" + (f" from {source}" if source != None else "") + "\n"
...
def upload_torrent(torrent_path: str) -> bool:
    ...
    if path.isfile(torrent_path):
        ...
        # file_len = path.getsize(torrent_path)
            # pieces = math.ceil(file_len / PIECE_SIZE)
            # last_piece_size = file_len - (pieces - 1) * PIECE_SIZE
        ...
        files.append(TorrentFileInfo(path.basename(torrent_path), path.getsize(torrent_path), piece_hashes))
    else:
        for root, _, file_names in os.walk(torrent_path, topdown=False):
            ...
            dirname = path.join(path.basename(torrent_path), 
            ...
            for file_name in file_names:
                ...
                local_torrent_file_name = path.join(path.basename(torrent_path), 
                                                    file_path[(len(path.normpath(torrent_path)) + 1):])
                
                log(f"Generating piece hashes for file {local_torrent_file_name}", "UPLOAD TORRENT")
                ...
                files.append(TorrentFileInfo(local_torrent_file_name, file_size, piece_hashes))
    
    log("Finished generating piece hashes, uploading torrent info to tracker...", "UPLOAD TORRENT")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        try:
            ...
            print("Failed to upload torrent info to the server. Check your network connection.")
            return False
        ...
        try:
            ...
            print("Failed to get response from the server. Check your network connection.")
            return False
        
        if req_type != RT_UPLOAD_RESPONSE:
            log(f"Unexpected Response from tracker, expected {RT_UPLOAD_RESPONSE}, received {req_type}", "UPLOAD TORRENT")
            print(f"Unexpected Response from tracker, expected {RT_UPLOAD_RESPONSE}, received {req_type}.")
            return False
        
        req: UploadTorrentResponse
        
        log(f"Torrent uploaded successfully. Torrent ID: {req.torrent_id}", "UPLOAD TORRENT")
        ...
        with torrent_infos_lock:
            torrent_infos[req.torrent_id] = TorrentInfo(req.torrent_id, dirs, files, PIECE_SIZE)
...
def download_torrent(torrent_id: str):
    ...
    try:
        ...
        with download_statuses_lock:
            download_statuses[torrent_id].set_error(f"Error while getting torrent info on {torrent_id}.\nError:\n\t{e.message}")
    ...
    if info.is_invalid():
        ...
        with download_statuses_lock:
            download_statuses[torrent_id].set_error(f"Error while downloading torrent {torrent_id}. Not found.")
            download_statuses[torrent_id].cancel()
        return
    ...
    if path.exists(partial_torrent_path):
        ...
        with download_statuses_lock:
            download_statuses[torrent_id].set_status("Loading existing partial files...")
        
        try:
            files_folders = os.listdir(partial_torrent_path)
        except BaseException as e:
            log(f"Failed to load existing partial files. Error: {e}. Exiting...", f"DOWNLOAD {torrent_id}")
            with download_statuses_lock:
                download_statuses[torrent_id].set_error(f"Failed to load existing partial files. Error: {e}")
                download_statuses[torrent_id].cancel()
            return
        ...
        for file_folder in files_folders:
            ...
            for piece_file in piece_files:
                ...
                if piece_index >= len(info.files[index].piece_hashes) or piece_index < 0:
                    continue
                ...
                if path.getsize(piece_path) != get_piece_size(info, index, piece_index):
                    try:
                        ...
                        log(f"Error trying to remove invalid piece file {piece_path}", f"DOWNLOAD {torrent_id}")
                    continue
                ...
                try:
                    ...
                    log(f"Error trying to read piece file {piece_path}", f"DOWNLOAD {torrent_id}")
                    try:
                        os.remove(piece_path)
                    except:
                        log(f"Error trying to remove invalid piece file {piece_path}", f"DOWNLOAD {torrent_id}")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        if not connect_socket_to_server(sock, True): # this only returns false if the exit event was set
            return
        try:
            sock.sendall(RegisterAsPeerRequest(torrent_id).to_bytes())
        except BaseException as e:
            log(f"Failed to register as seeder. Error: {e}. Exiting...", f"DOWNLOAD {torrent_id}")
            with download_statuses_lock:
                download_statuses[torrent_id].set_error(f"Failed to register as seeder. Error: {e}")
                download_statuses[torrent_id].cancel()
            return
    ...
    while not exit_event.is_set():
        ...
        log("Refreshing download peers...", f"DOWNLOAD {torrent_id}")
        with download_statuses_lock:
        ...
        if len(peers) == 0:
            with download_statuses_lock:
                download_statuses[torrent_id].set_status(f"No peers found. Retrying in 5 seconds...")
            time.sleep(5)
            continue
        ...
        log("Starting chunk download...", f"DOWNLOAD {torrent_id}")
        with download_statuses_lock:
        ...
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(download_file_piece, info, file, piece, peers) 
            ...
            for future in as_completed(futures):   
                ...
                try:
                    ...
                    log(f"Error downloading a piece. Error: {e}", f"DOWNLOAD {torrent_id}")
                    with download_statuses_lock:
                        download_statuses[torrent_id].set_error(f"Error downloading a piece. Error: {e}")
                ...
                if len(pending_download_files[file]) == 0:
                    ...
                    log(f"File {file} finished downloading, checking integrity and reconstructing...", f"DOWNLOAD {torrent_id}")
                    
                    file_name = path.join(f"downloads/{torrent_id}", info.files[file].file_name)
                    os.makedirs(path.dirname(file_name), exist_ok=True)
                    
                    reconstructed_bytes: bytes = b''
                    failed = False
                    ...
                    for piece_index in range(len(info.files[file].piece_hashes)):
                        ...
                        if not path.isfile(piece_path):
                            log(f"Found missing piece file ({file}, {piece_index}), should be at {piece_path}. Resubmitting to download queue...", f"DOWNLOAD {torrent_id}")
                            failed = True
                            fail_func(piece_index)
                            continue
                        
                        if path.getsize(piece_path) > PIECE_SIZE:
                            log(f"Found piece file ({file}, {piece_index}) at {piece_path} with invalid size. Resubmitting to download queue...", f"DOWNLOAD {torrent_id}")
                            try:
                                os.remove(piece_path)
                            except:
                                log(f"Failed to remove piece file {piece_path}.", f"DOWNLOAD {torrent_id}")
                            failed = True
                            fail_func(piece_index)
                            continue
                        ...
                        try:
                            ...
                            log(f"Failed to read piece file {piece_path}. Resubmitting to download queue...", f"DOWNLOAD {torrent_id}")
                            try:
                                os.remove(piece_path)
                            except:
                                log(f"Failed to remove piece file {piece_path}.", f"DOWNLOAD {torrent_id}")
                        ...
                        if not check_piece_hash(piece_data, info.files[file].piece_hashes[piece_index]):
                            log(f"Piece file {piece_path} has invalid hash. Resubmitting to download queue...", f"DOWNLOAD {torrent_id}")
                            try:
                                os.remove(piece_path)
                            except:
                                log(f"Failed to remove piece file {piece_path}.", f"DOWNLOAD {torrent_id}")
                        ...
                        if not failed:
                            reconstructed_bytes += piece_data
                    
                    if failed:
                        log(f"Some parts of file {file} couldn't be veryfied to be correct and will be downloaded again. File was not reconstructed.", f"DOWNLOAD {torrent_id}")
                        continue
                    ...
                    try:
                        ...
                        log(f"Failed to reconstruct file, cancelling download... Error: {e}", f"DOWNLOAD {torrent_id}")
                        with download_statuses_lock:
                            download_statuses[torrent_id].set_error(f"Failed to reconstruct file {file_name}. Error: {e}")
                            download_statuses[torrent_id].cancel_event.set()
                        executor.shutdown(wait=False, cancel_futures=True)
                        exited = True
                    finally:
                    ...
                    for piece_index in range(len(info.files[file].piece_hashes)):
                        ...
                        try:
                            ...
                            log(f"Failed to remove piece file {p_path}. Continuing...", f"DOWNLOAD {torrent_id}")
                            continue
                        finally:
                    ...
                    try:
                        ...
                        log(f"Failed to remove partial files folder {fldr}. Continuing...", f"DOWNLOAD {torrent_id}")
                # endregion
        ...
        if len(pending_download_files) == 0:
            ...
            try:
                ...
                log(f"Failed to remove partial download folder downloads/.partial/{torrent_id}", f"DOWNLOAD {torrent_id}")            
            break
...
def download_file_piece(torrent_info: TorrentInfo, file_index: int, piece_index: int, peers: list[str]) -> tuple[int, int] | None:
    ...
    log("Starting piece download...", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
    
    for peer in randomized_peers:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            log(f"Trying to connect to peer {peer}", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
            try:
                sock.connect((peer, CLIENT_CONNECTION_PORT))
            except:
                log(f"Failed to connect to peer {peer}", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
                continue
            
            log(f"Connected to peer {peer}", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
            
            with download_statuses_lock:
                download_statuses[torrent_info.id].set_download_source(file_index, piece_index, peer)
            
            try:
                sock.sendall(DownloadPieceRequest(torrent_info.id, file_index, piece_index).to_bytes())            
            except BaseException as e:
                log(f"Failed to send download request to peer {peer}. Error: {e}", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
                with download_statuses_lock:
                    download_statuses[torrent_info.id].set_download_source(file_index, piece_index, None)
                continue
            ...
            try:
                ...
                log(f"Failed to get response from peer {peer}. Error: {e}", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
                with download_statuses_lock:
                    download_statuses[torrent_info.id].set_download_source(file_index, piece_index, None)
                continue
            
        if req_type != RT_PIECE_DATA:
            log(f"Unexpected response from peer {peer}, expected {RT_PIECE_DATA}, received {req_type}", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
            with download_statuses_lock:
                download_statuses[torrent_info.id].set_download_source(file_index, piece_index, None)
            continue
        ...
        if req.request_status != ST_OK or req.piece_bytes == b'':
            log(f"Piece data not received from peer {peer}, request status: {status_str(req.request_status)}", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
            with download_statuses_lock:
                download_statuses[torrent_info.id].set_download_source(file_index, piece_index, None)
            continue
                
        if not check_piece_hash(req.piece_bytes, torrent_info.files[file_index].piece_hashes[piece_index]):
            log(f"Peer {peer} sent invalid piece", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
            with download_statuses_lock:
                download_statuses[torrent_info.id].set_download_source(file_index, piece_index, None)
            continue
        ...
        try:
            ...
            log(f"Failed to write piece file {piece_path}. Error: {e}", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
            with download_statuses_lock:
    ...
    log("Failed to download piece, could not find any suitable peer", f"DOWNLOAD {torrent_info.id} {file_index}, {piece_index}")
...
def connect_socket_to_server(socket: socket.socket, block_on_disconnect: bool = False) -> bool:
    ...
    try:
        ...
        while not exit_event.is_set():            
            log("Failed to connect to server. Trying again in 5 seconds...", "SERVER CONNECTION")
            time.sleep(5)
            try:
...
def check_file(torrent_info: TorrentInfo, file_index: int, file_path: str = None) -> bool:
    ...
    try:
        ...
        log(f"Error while checking file integrity for {file_path}. Error: {e}", "CHECK FILE")
        return False
    finally:
        close_file_with_lock(file_path)
...
def update_download_progress(torrent_id: str, file_index: int, piece_index: int, progress: float):
    with download_statuses_lock:
...
def get_torrent_info(torrent_id: str, block_on_disconnect: bool = False) -> TorrentInfo:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        try:
            ...
            if block_on_disconnect:
                log("Failed to send torrent request to the server after a successful connection. Retrying...", "GET TORRENT INFO")
        ...
        try:
            ...
            if block_on_disconnect:
                log("Failed to get torrent info after a successful connection to the server. Retrying...", "GET TORRENT INFO")
                return get_torrent_info(torrent_id, True)
            else:
                return TorrentInfo.invalid(torrent_id)
    ...
    if req_type != RT_TORRENT:
        if block_on_disconnect:
            log(f"Unexpected Response from tracker, expected {RT_TORRENT}, received {req_type}. Retrying...", "GET TORRENT INFO")
            return get_torrent_info(torrent_id, True)
        else:
            return TorrentInfo.invalid(torrent_id)
...
def get_torrent_peers(torrent_id: str, block_on_disconnect: bool = False) -> list[str]: 
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        try:
            ...
            if block_on_disconnect:
                log("Failed to send request to the server after a successful connection. Retrying...", "GET PEERS")
        ...
        try:
            ...
            if block_on_disconnect:
                log("Failed to get peers after a successful connection to the server. Retrying...", "GET PEERS")
                return get_torrent_peers(torrent_id, True)
            else:
                return []
    ...
    if req_type != RT_GET_PEERS_RESPONSE:
        if block_on_disconnect:
            log(f"Unexpected Response from tracker, expected {RT_GET_PEERS_RESPONSE}, received {req_type}. Retrying...", "GET PEERS")
            return get_torrent_peers(torrent_id, True)
        else:
            return []
...
def seeder_thread():
    ...
    while not exit_event.is_set():
        ...
        client_thread = Thread(target=handle_incoming_peer, args=[client, ip], daemon=False)
        client_thread.start()

def handle_incoming_peer(sock: socket.socket, ip: str):
    global uploading_count
    
    log(f"Incomming connection.", f"PEER {ip}")
    ...
    with sock:
        ...
        if req_type == RT_DOWNLOAD_PIECE:
            ...
            log(f"Peer is requesting upload of piece {req.piece_index} from file {req.file_index} in torrent {req.torrent_id}", f"PEER {ip}")
            
            with uploading_count_lock:
                if uploading_count >= UPLOAD_MAX:
                    log(f"Already at or above max upload capacity {UPLOAD_MAX}. Closing connection", f"PEER {ip}")
                    try:
            ...
            if not torrent_in_seeds:
                log(f"Peer requested piece from torrent {req.torrent_id}, which is not in the seeds list. Closing connection", f"PEER {ip}")
                try:
            ...
            log("Getting torrent info from server...", f"PEER {ip}")
            torrent_info = get_torrent_info(req.torrent_id)
            
            if torrent_info.is_invalid():
                log(f"Failed to get torrent info for torrent {req.torrent_id}. Closing connection", f"PEER {ip}")
                try:
                    sock.sendall(PieceDataResponse(ST_NOTFOUND, b'').to_bytes())
                except BaseException as e:
                    log(f"Failed to send response to peer. Error: {e}", f"PEER {ip}")
            ...
            if len(torrent_info.files) <= req.file_index or req.file_index < 0:
                log(f"Peer requested file index {req.file_index} which is out of bounds for torrent {req.torrent_id}. Closing connection", f"PEER {ip}")
                try:
                    sock.sendall(PieceDataResponse(ST_NOTFOUND, b'').to_bytes())
                except BaseException as e:
                    log(f"Failed to send response to peer. Error: {e}", f"PEER {ip}")
            ...
            if len(file.piece_hashes) <= req.piece_index or req.piece_index < 0:
                log(f"Peer requested piece index {req.piece_index} which is out of bounds for file {req.file_index} in torrent {req.torrent_id}. Closing connection", f"PEER {ip}")
                try:
                    sock.sendall(PieceDataResponse(ST_NOTFOUND, b'').to_bytes())
                except BaseException as e:
                    log(f"Failed to send response to peer. Error: {e}", f"PEER {ip}")
            ...
            for p in paths:
                log(f"Checking path {p}", f"PEER {ip}")
                if p == f"downloads/.partial/{req.torrent_id}":
                    ...
                    if path.getsize(piece_file_path) > PIECE_SIZE:
                        log(f"Piece file {piece_file_path} has an invalid size", f"PEER {ip}")
                    ...
                    try:
                        ...
                        log(f"Failed to read piece file {piece_file_path}. Error: {e}", f"PEER {ip}")
                        continue
                    finally:
                        close_file_with_lock(piece_file_path)
                    
                    if not check_piece_hash(piece_bytes, piece_hash):
                        log(f"Piece file {piece_file_path} has invalid hash", f"PEER {ip}")
                        continue
                    
                    log("Sending piece data to peer...", f"PEER {ip}")
                    ...
                    try:
                        ...
                        log(f"Failed to send piece data to peer. Closing the connection. Error: {e}", f"PEER {ip}")
                        return
                    finally:
                        with uploading_count_lock:
                            uploading_count -= 1
                    ...
                    log(f"Openning {req.file_index}, {req.piece_index} with lock")
                    
                    open_file_with_lock(file_path)  
                    
                    log(f"Getting {req.file_index}, {req.piece_index} data")
                    
                    try:
                        with open(file_path, "rb") as f:
                            f.seek(req.piece_index * torrent_info.piece_size)
                            # piece_size = get_piece_size(torrent_info, req.file_index, req.piece_index)
                            piece_bytes = f.read(PIECE_SIZE)
                    except BaseException as e:
                        log(f"Failed to read piece from file {file_path}. Error: {e}", f"PEER {ip}")
                        continue
                    finally:
                        close_file_with_lock(file_path)
                    
                    log(f"Checking {req.file_index}, {req.piece_index} hash")
                    
                    if not check_piece_hash(piece_bytes, piece_hash):
                        log(f"Piece from file {file_path} has invalid hash", f"PEER {ip}")
                        continue
                    
                    log(f"Setting {req.file_index}, {req.piece_index} timeout")
            ...
            log("Failed to find piece data", f"PEER {ip}")
...
def server_communication_thread():
    ...
    while not exit_event.is_set():
        ...
        server_thread = Thread(target=handle_server_request, args=[server_sock], daemon=True)
        server_thread.start()
...
def handle_server_request(sock: socket.socket):
    ...
    with sock:
        ...
        if req_type == RT_GET_CLIENT_TORRENTS:
            log("Server is requesting torrents currently being seeded", "SERVER CONNECTION")
            ...
            try:
                ...
                log(f"Failed to send response to server. Error: {e}", "SERVER CONNECTION")
                return
...
def download_command(args: list[str]):
    ...
    download_thread = Thread(target=download_torrent, args=[args[0]], name="download", daemon=False)
...
def list_seeds(args: list[str]):
    ...
    with seeds_lock:
        ...
        print("\n".join(map(lambda s: f"{s}: {available_seeds[s]}", available_seeds.keys())))
...
def show_download_log(torrent_id: str):
    while not leave_download_event.is_set() and not exit_event.is_set():
        ...
        with download_statuses_lock:
            ...
            if download_statuses[torrent_id].finished or download_statuses[torrent_id].cancelled:
                return
...
def join_download_command(args: list[str]):
    ...
    if len(args) > 0 and args[0] in torrents:
        ...
        while True:
            try:
                inp = input(f"Enter the index of the download to join [1 - {len(torrents)}] write 'clear' to remove finished or cancelled downloads or 'exit' to leave: ").lower().strip()
            ...
            if index < 1 or index > len(torrents):
                print(f"Invalid index. Please enter a number between 1 and {len(torrents)}.")
                continue
    ...
    log_thread = Thread(target=show_download_log, args=[torrent_id], name="download_log", daemon=True)
    log_thread.start()
...
def cancel_download_command(args: list[str]):
    if len(args) == 0:
        print("Wrong number of arguments in 'cancel-download' command, use 'cancel-download help' for usage info.")
        return
...
def seed_command(args: list[str]):
    if len(args) == 0 or (len(args) < 2 and args[0] != "help"):
        print("Wrong number of arguments in 'seed' command, use 'seed help' for usage info.")
        return
    ...
    with seeds_lock:
        if args[1] in available_seeds and path.dirname(args[0]) in available_seeds[args[1]]:
...
def main():
    ...
    server_comms = Thread(target=server_communication_thread, name="server_comms", daemon=True)
    server_comms.start()
    
    seeder = Thread(target=seeder_thread, name="seeder", daemon=True)
    seeder.start()
    
    reconnection_thread = Thread(target=attempt_reconnection_thread, name="reconnection", daemon=True)
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as login_socket:
        if not connect_socket_to_server(login_socket):
            print(f"Failed to connect to server, the server may be unavailable or your internet connection may be down. Exiting...")
